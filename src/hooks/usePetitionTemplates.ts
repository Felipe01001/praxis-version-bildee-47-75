...
OAB/[UF] nº [número]`;
  ...
  const clearAllTemplates = async () => {
    try {
      console.log('Clearing all templates and files...');
      toast.loading('Removendo todos os modelos e arquivos...');
      
      // Buscar todos os arquivos para deletar do storage
      const { data: allFiles } = await supabase
        .from('petition_template_files')
        .select('storage_path');

      // Deletar todos os arquivos do storage em lotes
      if (allFiles && allFiles.length > 0) {
        ...
        if (storagePaths.length > 0) {
          // Deletar em lotes de 100 arquivos por vez
      ...
      // Deletar todos os registros de arquivos
      const { error: filesError } = await supabase
        .from('petition_template_files')
        .delete()
      ...
      if (templatesError) {
        console.error('Error deleting all templates:', templatesError);
        throw templatesError;
      }

      console.log('All templates and files cleared successfully');
      toast.dismiss();
      toast.success('Todos os modelos e arquivos foram removidos com sucesso');
      
      // Limpar lista local
      setTemplates([]);
    } catch (err) {
      console.error('Erro ao limpar todos os modelos:', err);
      toast.dismiss();
      toast.error('Erro ao limpar modelos');
      throw err;
    }
  };
