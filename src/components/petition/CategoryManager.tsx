...
import { Plus, FolderOpen, FileText, ChevronDown, ChevronRight, Upload, Download, ExternalLink, Eye } from 'lucide-react';
...
interface CategoryManagerProps {
  ...
  onMultipleFileUpload?: (templateId: string, files: File[]) => void;
  templatesFromDb: Array<{
    id: string;
    ordem: string;
    titulo: string;
    tema: string;
...
export const CategoryManager = ({ onAddTemplate, onFileUpload, onMultipleFileUpload, templatesFromDb }: CategoryManagerProps) => {
  const { handleMouseEnter, handleMouseLeave } = useHoverEffect();
  
  // Define categories structure first - TODAS AS CATEGORIAS COMPLETAS
  const [categories, setCategories] = useState<Category[]>([
    ...
    },
    {
      ...
      items: [
        ...
        { id: '19.3', title: 'VEÍCULO – FURTO do Veículo em ESTACIONAMENTO – irrecuperável – Indenização material', order: '19.3', files: [] },
        { id: '19.4.0', title: 'VEÍCULO – Réu PF - Compra e venda – VÍCIO OCULTO – Ressarcimento', order: '19.4.0', files: [] },
        ...
        { id: '19.15', title: 'VEÍCULO – ZERO KM – DEFEITO NO PRODUTO – GARANTIA RECUSADA', order: '19.15', files: [] },
      ]
    },
    {
      id: '20',
  ...
  // Integrar dados do banco com as categorias hardcoded
  const getCategoriesWithFiles = () => {
    const categoriesWithFiles = categories.map(category => ({
      ...category,
      items: category.items.map(item => {
        // Buscar template do banco com base na ordem
        const templateFromDb = templatesFromDb.find(t => t.ordem === item.order);
        
        return {
          ...item,
          files: templateFromDb?.files.map(f => ({
            id: f.id,
            name: f.arquivo_nome,
            type: f.tipo,
            url: f.arquivo_url,
  ...
  const categoriesWithDbData = getCategoriesWithFiles();
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>, itemOrder: string) => {
    const files = Array.from(event.target.files || []);
    if (files.length === 0) return;

    // Verificar se todos os arquivos são PDF ou DOCX
    const invalidFiles = files.filter(file => !file.name.endsWith('.pdf') && !file.name.endsWith('.docx'));
    ...
    try {
      console.log('Iniciando upload para ordem:', itemOrder);
      
      // Buscar se já existe um template com essa ordem
      let existingTemplate = templatesFromDb.find(t => t.ordem === itemOrder);
      
      if (!existingTemplate) {
        // Se não existe template, criar um novo
        const foundItem = getCategoryItemByOrder(itemOrder);
        if (!foundItem) {
          toast.error('Item não encontrado');
          return;
        }
        
        console.log('Criando novo template para:', foundItem);
        await onAddTemplate(foundItem.categoryTitle, foundItem.itemTitle, itemOrder);
        
        // Aguardar um pouco para o template ser criado
      ...
      for (const file of files) {
        ...
        await onFileUpload(itemOrder, file);
      }
  ...
  const getCategoryItemByOrder = (itemOrder: string) => {
    for (const category of categories) {
      const item = category.items.find(i => i.order === itemOrder);
      if (item) {
        return {
  ...
  const handleDownloadFile = (fileUrl: string, fileName: string) => {
    console.log('Baixando arquivo:', fileName, 'URL:', fileUrl);
  ...
  const handleViewFile = (fileUrl: string, fileName: string) => {
    console.log('Visualizando arquivo:', fileName, 'URL:', fileUrl);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {categoriesWithDbData.map((category) => (
                    ...
                    {category.items.map((item) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {/* Botões de visualização e download para cada arquivo */}
                          {item.files.map((file) => (
                          ...
                          <label className="cursor-pointer">
                            <input
                              ...
                              multiple
                              className="hidden"
                              onChange={(e) => handleFileUpload(e, item.order)}
                            />
                            <Button variant="ghost" size="sm" asChild>
                              <span title="Adicionar arquivo(s) - múltiplos arquivos permitidos" className="h-8 w-8 p-0">
