...
export const CasesTab = ({
  ...
  const handleUpdateStatus = async (caseItem: Case, newStatus: 'open' | 'completed') => {
  ...
  return <div className="space-y-6 py-[51px]">
      ...
      {cases.length > 0 ? <div className="grid gap-4">
          {cases.map(caseItem => <CaseCard key={caseItem.id} case={caseItem} onEdit={setEditingCase} onDelete={handleDeleteCase} onUpdateStatus={handleUpdateStatus} />)}
        </div> : <Card>
