...
export default function NotificationCenter() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      } = await supabase.from('scheduled_notifications').select('*').eq('user_id', user.id).order('scheduled_for', {
        ascending: true
      });
      if (scheduledNotifications) {
        ...
        const pendingCount = scheduledNotifications.filter(n => n.status === 'pending' && new Date(n.scheduled_for) <= new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) // próximos 7 dias
        ).length;
      ...
      const {
        data: logs
      } = await supabase.from('notification_logs').select('*').eq('user_id', user.id).order('sent_at', {
        ascending: false
      }).limit(20);
      if (logs) {
      ...
      } = await supabase.from('notification_settings').select('*').eq('user_id', user.id).order('created_at', {
        ascending: false
      });
      ...
      const {
        data: tasks
      } = await supabase.from('tasks').select('id, title, endDate, status').eq('userId', user.id).not('status', 'eq', 'completed').gte('endDate', new Date().toISOString()).lte('endDate', new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString()) // próximas 2 semanas
      .order('endDate', {
        ascending: true
      });
      if (tasks) {
  ...
  const saveDefaultSettings = async () => {
    ...
    try {
      ...
      if (defaultSettings) {
        ...
        const {
          error
        } = await supabase.from('notification_settings').update(settingsData).eq('id', defaultSettings.id);
        if (error) throw error;
      } else {
        // Criar nova configuração
        const {
          error
        } = await supabase.from('notification_settings').insert(settingsData);
        if (error) throw error;
      }
        ...
        <Tabs defaultValue="upcoming" className="w-full">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 gap-1">
            <TabsTrigger value="upcoming" className="text-xs md:text-sm">
              Próximas Tarefas
              {upcomingTasks.length > 0 && <Badge variant="secondary" className="ml-1 md:ml-2 text-xs">
                  {upcomingTasks.length}
                </Badge>}
            </TabsTrigger>
            <TabsTrigger value="scheduled" className="text-xs md:text-sm">
              Notificações Agendadas
              {notifications.filter(n => n.status === 'pending').length > 0 && <Badge variant="secondary" className="ml-1 md:ml-2 text-xs">
          ...
          <div className="mt-4 max-h-[60vh] overflow-y-auto">
            <TabsContent value="upcoming" className="space-y-3">
              {loading ? <div className="text-center py-8">Carregando...</div> : upcomingTasks.length > 0 ? upcomingTasks.map(task => <Card key={task.id} className="p-4 py-[10px] my-[23px] mx-0">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={newDaysBefore.toString()} onValueChange={value => setNewDaysBefore(parseInt(value))}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch checked={newEmailEnabled} onCheckedChange={setNewEmailEnabled} />
                  </div>
                </div>
            ...
            <TabsContent value="scheduled" className="space-y-3">
              {loading ? <div className="text-center py-8">Carregando...</div> : notifications.length > 0 ? notifications.map(notification => <Card key={notification.id} className="p-4">
            ...
            <TabsContent value="history" className="space-y-3">
              {loading ? <div className="text-center py-8">Carregando...</div> : notificationLogs.length > 0 ? notificationLogs.map(log => <Card key={log.id} className="p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                        ...
                        {log.error_message && <p className="text-xs text-red-600 mt-1">
