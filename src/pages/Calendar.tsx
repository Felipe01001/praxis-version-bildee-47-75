...
const Calendar = () => {
  ...
  // Calculate tasks by date with creation/due date differentiation
  const tasksByDate = selectedDate ? tasksWithDate.filter(task => isSameDay(task.startDateTime, selectedDate) || isSameDay(task.endDateTime, selectedDate)).map(task => ({
    ...task,
    isCreationDate: isSameDay(task.startDateTime, selectedDate),
    isDueDate: isSameDay(task.endDateTime, selectedDate)
  })) : [];
  ...
  const handleTaskStatusUpdate = (taskId: string, newStatus: TaskStatus) => {
    ...
    if (task) {
      ...
      toast.success(`Status da tarefa atualizado para ${TASK_STATUS_LABELS[newStatus]}`);
    }
  };
  ...
  const KanbanColumn = ({
    ...
    tasks: Array<Task & {
      ...
      minHeight: '200px'
        ...
        {tasks.length > 0 ? tasks.map(task => <Card key={task.id} className={`w-full cursor-pointer transition-all hover:shadow-md duration-200 ${isCloseToDueDateOrOverdue(task) && status !== 'completed' ? 'border-amber-200 bg-amber-50/50' : ''}`} onClick={() => handleTaskClick(task)} onMouseEnter={e => handleMouseEnter(e, 0.5)} onMouseLeave={handleMouseLeave}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
        ...
        <div className="flex items-center space-x-2">
          <SubscriptionAccessWrapper action="criar uma nova tarefa">
            <Button variant="outline" onClick={() => setIsAddingTask(true)} className="theme-primary-button text-base">
      ...
      <Tabs value={view} onValueChange={v => setView(v as 'day' | 'month' | 'kanban')}>
        ...
        <TabsContent value="month">
          <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
            {/* Calendar Card - Full width on mobile/tablet, optimized for lg+ */}
            <div className="w-full lg:w-1/3 lg:min-w-[350px]">
              <Card className="w-full">
                <CardHeader className="flex flex-row items-center justify-between pb-4">
                  <CardTitle className="font-semibold text-lg">Calendário</CardTitle>
                  <div className="flex items-center gap-1">
                ...
                <CardContent className="p-4 rounded-md bg-stone-50 py-px px-[53px]">
                ...
                }} className="w-full mx-0 my-0 px-[4px]" />
            ...
            {/* Tasks List Card - Full width on mobile/tablet, flex-1 on lg+ */}
            <div className="w-full lg:flex-1">
              <Card className="w-full h-full">
                ...
                <CardContent className="p-4">
                  {sortedTasks.length > 0 ? <div className="space-y-4 max-h-[calc(100vh-400px)] lg:max-h-[calc(100vh-300px)] overflow-y-auto">
                      {sortedTasks.map(task => {
                    const startTime = format(task.startDateTime, 'HH:mm');
                    const endTime = format(task.endDateTime, 'HH:mm');
                    const statusColor = getStatusColor(task.status);
                    const isCreationDay = task.isCreationDate && !task.isDueDate;
                    const isDueDay = task.isDueDate && !task.isCreationDate;
                    const isBothDays = task.isCreationDate && task.isDueDate;
                    return <div key={`${task.id}-${isCreationDay ? 'created' : isDueDay ? 'due' : 'both'}`} className="border-l-4 pl-4 py-3 cursor-pointer transition-all duration-200 rounded-r-md bg-card hover:bg-muted/50" style={{
                      borderLeftColor: statusColor
                    }} onClick={() => handleTaskClick(task)} onMouseEnter={e => handleMouseEnter(e, 0.5)} onMouseLeave={handleMouseLeave}>
                           <div className="flex flex-col gap-2">
                             <div className="flex-1">
                               <div className="flex items-center gap-2 mb-1 flex-wrap">
                                 <h3 className="font-medium text-sm sm:text-base line-clamp-2">{task.title}</h3>
                                {isCreationDay && <Badge variant="outline" className="text-xs bg-blue-50 text-blue-700 border-blue-200">
                                    📅 Criada hoje
                                  </Badge>}
                                {isDueDay && <Badge variant="outline" className="text-xs bg-amber-50 text-amber-700 border-amber-200">
                                    ⏰ Vence hoje
                                  </Badge>}
                                {isBothDays && <Badge variant="outline" className="text-xs bg-green-50 text-green-700 border-green-200">
                                    🎯 Criada e vence hoje
                                  </Badge>}
                              </div>
                               <p className="text-xs sm:text-sm text-muted-foreground">{task.clientName}</p>
                             </div>
                             <div className="flex items-center justify-between gap-2">
                               <span className="text-xs rounded-full px-2 sm:px-2.5 py-0.5 text-center" style={{
                           ...
                           <div className="flex items-center gap-2 sm:gap-4 mt-2 text-xs sm:text-sm text-muted-foreground flex-wrap">
                            {!isBothDays && <div className="flex items-center gap-1">
                                {isCreationDay && <>
                                    <CalendarIcon className="h-3 w-3" />
                                    <span>Prazo: {format(task.endDateTime, 'dd/MM')}</span>
                                  </>}
                                {isDueDay && <>
                                    <CalendarIcon className="h-3 w-3" />
                                    <span>Criada: {format(task.startDateTime, 'dd/MM')}</span>
                                  </>}
                              </div>}
                          </div>
                          
                          {task.description && <p className="mt-2 text-xs sm:text-sm text-muted-foreground line-clamp-2">{task.description}</p>}
                        </div>;
                  })}
              ...
              <CardContent>
                <div className="space-y-4">
                  {sortedTasks.length > 0 ? <div className="divide-y divide-gray-100">
                      ...
                      const isDueDay = task.isDueDate && !task.isCreationDate;
                      const isBothDays = task.isCreationDate && task.isDueDate;
                      return <div key={`${task.id}-${isCreationDay ? 'created' : isDueDay ? 'due' : 'both'}`} className="py-4 cursor-pointer transition-all duration-200 rounded-md hover:bg-muted/50" onClick={() => handleTaskClick(task)} onMouseEnter={e => handleMouseEnter(e, 0.5)} onMouseLeave={handleMouseLeave}>
                            ...
                            backgroundColor: statusColor
                              ...
                              <div className="flex-1">
                                ...
                                {!isBothDays && <div className="mt-2 text-sm text-muted-foreground">
                                    {isCreationDay && <span>📅 Prazo: {format(task.endDateTime, 'dd/MM/yyyy')}</span>}
                                    {isDueDay && <span>📝 Criada: {format(task.startDateTime, 'dd/MM/yyyy')}</span>}
                                  </div>}
        ...
        <TabsContent value="kanban">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 h-[calc(100vh-400px)] overflow-hidden">
