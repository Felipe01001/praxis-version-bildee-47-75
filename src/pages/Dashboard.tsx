...
import { debugUserSession, compareUserSessions } from '@/utils/debugAuth';
...
const Dashboard = () => {
  const { clients = [], cases = [], tasks = [], judicialProcesses = [], petitions = [], addTask, updateTask } = usePraxisContext();
  ...
  const openTasks = tasks?.filter(t => t.status === 'in-progress').length || 0;
  const delayedTasks = tasks?.filter(t => t.status === 'delayed').length || 0;
