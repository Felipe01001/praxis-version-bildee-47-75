...
import { Category, Status, ClientStatus, Client } from '@/types';
...
const ClientList = () => {
  ...
  const filteredClients = clients.filter(client => {
    ...
    // Check if client status matches filter
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <SubscriptionAccessWrapper action="criar um novo cliente">
          <Button onClick={handleAddClient} className="theme-primary-button flex items-center gap-2">
        ...
        <table className="w-full border-collapse rounded-md overflow-hidden">
          <thead>
            <tr className="bg-muted text-muted-foreground">
              ...
              <th className="py-3 px-4 text-center">Status do Cliente</th>
              <th className="py-3 px-4 text-center">Status Atendimento</th>
            </tr>
          </thead>
          <tbody>
            {filteredClients.length > 0 ? (
              filteredClients.map((client) => {
                const caseStatus = getClientMostRecentStatus(client.id);
                ...
                return (
                  ...
                  >
                    <td className="py-3 px-4">
                      <span className="text-praxis-olive hover:underline font-medium">
                        {client.name}
                      </span>
                    </td>
