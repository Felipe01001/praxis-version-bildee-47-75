...
const JudicialProcessesList = () => {
  ...
  useEffect(() => {
    ...
    const filtered = judicialProcesses.filter(process => {
      const matchesSearch = !searchTerm || process.numeroProcesso?.toLowerCase().includes(searchTerm.toLowerCase()) || process.tribunal?.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <SubscriptionAccessWrapper action="pesquisar um processo judicial">
            <Button asChild className="theme-primary-button">
      ...
      <Card>
        <CardHeader>
          ...
          <div className="relative mt-4">
            ...
            <Input placeholder="Buscar por número de processo ou tribunal..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
          </div>
        </CardHeader>
        <CardContent>
          {filteredProcesses.length > 0 ? <div className="grid grid-cols-1 gap-4">
              {filteredProcesses.map(process => <div key={process.id} className="border rounded-lg p-4 hover:border-praxis-olive hover:bg-muted/30 cursor-pointer transition-colors" onClick={() => navigate(`/judicial-processes/${process.id}`)}>
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-x-8 gap-y-2 text-sm text-muted-foreground">
                        ...
                        {process.clientId && <div className="flex items-center gap-1">
                            ...
                            <Link to={`/clients/${process.clientId}`} className="text-praxis-olive hover:underline" onClick={e => e.stopPropagation()}>
                    ...
                    <div>
                      <Button variant="ghost" size="sm" className="flex items-center gap-1" onClick={e => {
                  e.stopPropagation();
