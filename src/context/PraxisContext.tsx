...
import { Client, Case, Attachment, Task, JudicialProcess, Petition, TaskStatus } from '../types';
...
const transformTaskToDB = (task: Task, isUpdate: boolean = false) => {
  const baseTask = {
    ...
    updates: JSON.parse(JSON.stringify(task.updates || []))
  };

  // For updates, don't include updatedAt, id, or createdAt (let the database handle them)
  if (isUpdate) {
    return baseTask;
  }

  // For inserts, include id and createdAt
  return {
    ...baseTask,
    id: task.id,
...
export const PraxisProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadAllData = async () => {
    ...
    try {
      console.log('=== PRAXIS CONTEXT DEBUG ===');
      ...
      console.log('Attachments:', attachmentsRes.error ? `ERRO: ${attachmentsRes.error.message}` : `${attachmentsRes.data?.length || 0} registros`);
      console.log('Judicial Processes:', judicialProcessesRes.error ? `ERRO: ${judicialProcessesRes.error.message}` : `${judicialProcessesRes.data?.length || 0} registros`);
      console.log('Petitions:', petitionsRes.error ? `ERRO: ${petitionsRes.error.message}` : `${petitionsRes.data?.length || 0} registros`);
  ...
  // Check and update delayed tasks
  const checkAndUpdateDelayedTasks = async () => {
    if (!user) return;

    const now = new Date();
    const tasksToUpdate: Task[] = [];

    // Find tasks that should be marked as delayed
    tasks.forEach(task => {
    ...
    if (tasksToUpdate.length > 0) {
      console.log(`Encontradas ${tasksToUpdate.length} tarefas para marcar como atrasadas`);
      ...
      if (tasksToUpdate.length > 0) {
        toast.info(`${tasksToUpdate.length} tarefa(s) foi(ram) marcada(s) como atrasada(s)`);
      }
    }
  };

  // Auto-check for delayed tasks every 5 minutes
  useEffect(() => {
    ...
    const interval = setInterval(checkAndUpdateDelayedTasks, 5 * 60 * 1000);
    
    return () => clearInterval(interval);
  }, [user, tasks.length]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const addTask = async (taskData: Omit<Task, 'id' | 'createdAt'>) => {
    ...
    const taskForDB = transformTaskToDB(newTask, false); // Pass false for isUpdate
  ...
  const updateTask = async (id: string, task: Task) => {
    ...
    const { data, error } = await supabase
      .from('tasks')
      .update(transformTaskToDB(task, true)) // Pass true for isUpdate
      .eq('id', id)
      .eq('userId', user.id)
      .select()
      .single();
    ...
    // Update local state with the task that includes the database-generated updatedAt
    const updatedTask = { ...task, updatedAt: data?.updatedAt || new Date().toISOString() };
    setTasks(prev => prev.map(t => t.id === id ? updatedTask : t));
    console.log('Tarefa atualizada:', { id, status: updatedTask.status });
